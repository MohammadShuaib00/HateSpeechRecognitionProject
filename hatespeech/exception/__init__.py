import os
import sys
import traceback

def error_message_details(error, error_detail: sys):
    _, _, exc_tb = error_detail.exc_info()
    file_name = exc_tb.tb_frame.f_code.co_filename  # Get the file name where the error occurred
    tb_lineno = exc_tb.tb_lineno  # Get the line number where the error occurred
    error_message = "Error occurred in python script name [{0}] at line number [{1}] with error message [{2}]".format(
        file_name, tb_lineno, str(error)
    )

    return error_message

class CustomException(Exception):
    def __init__(self, error_message, error_detail: sys):
        super().__init__(error_message)  # Correct way to call the base class constructor
        self.error_message = error_message_details(
            error_message, error_detail=error_detail
        )

    def __str__(self):
        return self.error_message


